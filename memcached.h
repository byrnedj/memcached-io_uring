...
struct settings {
    ...
    bool use_cas;
    bool use_io_uring;
    bool use_io_uring_registered_buffers;
    int io_uring_depth;
    int io_uring_buf_count;
    ...
    /* Huge page settings */
    bool use_hugepages;         /* Enable huge page allocations */
    bool hugepage_thp_fallback; /* Fall back to transparent huge pages */
    int hugepage_size;          /* Preferred huge page size: 0=auto, 2=2MB, 1024=1GB */
#ifdef PROXY
    bool proxy_enabled;
...
typedef struct {
    ...
    int io_uring_fd;            /* io_uring fd for this thread */
    ...
    struct event io_uring_ev;   /* event for io_uring ring */	
    ...
    struct io_uring_buf_ring *buf_ring;  /* provided buffer ring */
    char *io_uring_buf_mem;       /* backing memory for provided buffers */
    int io_uring_buf_count;       /* number of provided buffers */
...
enum io_uring_op_type { OP_ACCEPT, OP_RECV, OP_SEND };

struct io_uring_op_ctx {
    conn                *c;        /* owning connection              */
    enum io_uring_op_type         type;     /* what the SQE is doing          */
    void (*handler)(struct io_uring_op_ctx *op, int res, unsigned int cqe_flags);
                                     /* completion callback          */
    bool using_provided_buf;         /* true if using provided buffers */
};
